...
- **Sending:** the composer is an `AXTextArea` titled with the chat name. The helper checks the bound chat is open, replaces existing composer text with the generated reply, then brings WeChat to the front if needed and verifies the same chat and composer are still active. It sends an ordinary system-level Return and waits for a new `MeSaid:<text>` row. If the exact reply remains in the composer, it can try that Return once more; it never retries after the composer clears or the result is uncertain.
  - Failures before Return (wrong chat) are retried a few times.
...
- **Cost of a snapshot.** The helper keeps the chat's list and composer between snapshots and looks them up again only when they stop answering (a closed window, a restarted WeChat), and returns only the newest 60 rows, with where they start in the whole list. 4.x keeps a placeholder for every row scrolled past, so reading them all got slower as the day went on. Because 4.x only appends, that start tells the channel exactly how many rows are new; guessing from the rows alone fails when most of the window is blank placeholders and several messages arrive at once (found in review, covered by a test).
- **Some 4.1.x installs collapse the tree.** In some environments, after WeChat's main renderer loads, Accessibility exposes only the window frame and its close/full-screen/minimize buttons, even with permissions enabled. See this [4.1.13 report](https://github.com/huj28-creator/wechat-fastbridge/issues/1). DearByte now falls back to local Vision OCR of the visible WeChat window for text; this needs Screen Recording permission and the window on the current desktop. The OCR fallback does not read photos.
- **Accessibility rows omit senders in 4.x.** Message rows are `AXStaticText` with identifier `chat_bubble_item_view` and the bubble's text as title, with no sender. For configured group chats, the helper supplements visible rows with OCR and associates a short nickname line above a left-side bubble. Accessibility text confirms both the message bubble and nickname candidate so a detached nickname is not treated as a message; if OCR cannot confirm the bubble, the sender stays unknown. OCR is limited to the visible WeChat window and isn't saved. Time labels have no identifier; rows scrolled out of view stay in the list as empty `virtual_cell`s.
- **Who sent it.** In visual snapshots, right-aligned bubbles are 小拜's. In configured group chats, OCR tries to pair a short nickname above a left-side bubble; if it cannot read one, the speaker is explicitly unknown. Accessibility rows remain the source for the text, and OCR is used only when its newest text matches.
- **Who sent it.** In visual snapshots, right-aligned bubbles are 小拜's. In configured group chats, OCR tries to pair a short nickname above a left-side bubble; if it cannot read one, the speaker is explicitly unknown. The channel also remembers what 小拜 sent in the last 10 minutes; matching bubbles (ignoring spaces and emoji codes like `[白眼]`) are hers even if WeChat repeats a row after a refresh.
- **Confirming a send.** The helper verifies the bound chat and exact composer text, presses Return with WeChat in front, then confirms that the composer cleared. If the result is uncertain, it does not blindly resend; the channel keeps the bubble marked as a possible echo.
...
- **Remaining 4.x limit:** sender OCR depends on a visible nickname above the bubble and can return unknown when the label is clipped or unreadable. Messages typed as 小拜 on the phone may still be ambiguous when Accessibility doesn't expose alignment.
...
- **Group chats:** supported only when the chosen conversation is explicitly marked as `"type": "group"`; sender names are kept as prompt context. OCR can still leave a sender unknown when its label is clipped or unreadable.
