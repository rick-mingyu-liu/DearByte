...
7. **Timing:** in direct chats, the reading pause grows with the message: about 0.6 s for 「在吗」, up to 3 s for a long message or a photo (model time counts toward it; changed from a flat 1.5–3.5 s after friends found replies slow). Group replies skip that added pause. Later bubbles use about 50 ms per character, capped at 1.6 s, with ±25% jitter; groups also skip this typing pause.
