...
适配 WeChat for Mac 4.x（在 4.1.13 上测过）和 3.8.4，英文界面，使用辅助功能读取和发送消息。部分 4.1.x 版本不开放聊天控件时，小拜会改用本机屏幕识别可见文字；备用方式只用于文字聊天，微信主窗口要显示在当前桌面，并给终端打开“屏幕与系统录音”权限（系统设置 → 隐私与安全性）。图片识别仍需可用的辅助功能控件。聊天要停在最底部。同一时间只能开一个小拜（第二个会拒绝启动），否则两个会互相回复。此方案用于实验演示，存在账号受限风险，请使用测试账号。
...
4. 在微信主窗口打开指定的个人聊天或群聊，滚动到底部。不要关闭主窗口，也不要将聊天拆到独立窗口。
...
首次运行创建的 `data/contacts.json` 已被 Git 忽略。该文件目前只支持一个聊天对象，可以是个人或群聊；群聊要在对象中设置 `"type": "group"`。如微信可能显示不同备注或群名，请列出所有名称。修改后重启。

切到其他聊天时，小拜会等待。启动前已有的消息不会被回复；个人聊天发现第二位发言者时会暂停。指定为群聊后，会回复群里所有可读取的新消息，不要求 @，也不会主动在群里开话题。个人聊天里约 1 秒内连续到达的消息会合并为一轮；群聊消息分开发起回复，最多同时处理 4 条，慢的一条不会挡住其他群友。群友昵称只用于分清说话的人，不会单独当作消息回复；纯报名字时会记住、不发确认消息。自动回复会替换微信输入框里已有的文字草稿，再发送生成的回复。一条气泡发送结果不确定时，会跳过它并尝试发送后续气泡，不会重发不确定的那条。模型 18 秒仍未完成时，会取消请求、发送简短兜底回复，再处理排队的新消息；读取助手启动后连续无响应 12 秒会重启。
...
1. 在登录小拜账号的 Mac 微信主窗口中，打开你希望小拜回复的聊天。
...
要添加同一个聊天的其他显示名称，编辑项目根目录下的 `data/contacts.json`。设置群聊时加上 `"type": "group"`：
...
这里的 `type` 可写 `direct` 或 `group`；群聊需写 `group`。`names` 匹配聊天顶部显示的名称，不是微信号；`id` 是内部标识。示例文件见 [contacts.example.json](../contacts.example.json)。

保存后退出并重新运行 `npm run dearbyte`。文件已存在时，`--chat` 不会追加或覆盖名单，必须直接编辑文件。当前仅支持一个聊天对象；群聊里会回复新消息，且不会主动发起聊天。历史与记忆按聊天隔离尚未实现。
