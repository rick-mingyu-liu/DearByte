...
export async function handleUpdate(d: InboxDeps, u: Update): Promise<string | null> {
  ...
  if (!Number.isInteger(id) || id <= 0) {
    await d.bot.answerCallback(q.id, "That button is no longer valid.");
  ...
  if (type === "fb" && (choice === "u" || choice === "n")) {
    ...
    await d.bot.answerCallback(q.id, saved ? (feedback === "useful" ? "Thanks, noted." : "Got it, I'll learn from that.") : "I can't find that alert.");
  ...
  if (type === "ap" && (choice === "y" || choice === "n")) {
    ...
    await d.bot.answerCallback(q.id, outcome.slice(0, 200));
    // The edit is cosmetic: the decision is already stored, so a failed edit changes nothing.
