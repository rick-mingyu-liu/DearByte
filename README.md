...
微信接入通过 macOS 辅助功能操作桌面客户端，当前针对 **WeChat for Mac 4.x（或 3.8.4）、英文界面、一个指定的个人聊天或群聊**。群聊需在 `data/contacts.json` 中将 `type` 设为 `group`；群聊不会主动发消息。部分 4.1.x 版本不开放聊天控件时，会改用本机屏幕识别读取可见文字；这种备用方式需要微信窗口显示在当前桌面，并给终端打开“屏幕与系统录音”权限。自动回复会替换输入框里已有的文字草稿，再发送生成的回复。备用方式目前用于文字聊天；图片识别仍需可用的辅助功能控件。需要辅助功能权限和 Swift；图片另需配置目录。

这是一条实验性演示路径，存在账号受限风险。请使用测试账号。配置步骤、草稿模式、暂停与恢复见[中文使用指南](docs/guide.zh-CN.md#微信接入)。

#### 设置联系人白名单

在登录小拜账号的 Mac 微信中，打开希望小拜回复的聊天。将聊天顶部显示的完整名称填入命令（不是微信号）：
...
如果同一个聊天顶部可能显示不同名称，编辑 `data/contacts.json`，把可能显示的名称加入 `names`。回复群聊时将 `type` 设为 `group`；默认是个人聊天：
...
将示例名称替换为同一聊天的实际显示名称。群聊中会回复新消息，但不会主动发消息。保存后重启程序。文件已存在时，`--chat` 不会追加或覆盖名单，需要直接编辑文件。

**当前仅支持一个聊天对象**，可以是个人或一个群聊；不要添加第二个联系人对象，因为历史与记忆尚未按聊天隔离。该文件已被 Git 忽略。更多说明见[联系人设置指南](docs/guide.zh-CN.md#设置允许回复的联系人)。
