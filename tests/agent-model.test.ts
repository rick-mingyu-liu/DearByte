...
import { ClaudeAgentModel, type AgentRequest } from "../src/agent/model.ts";
...
test("sends adaptive thinking, default fallbacks and a cached system prompt", async () => {
  const { client, sent } = clientReturning(toolUseMessage);
  await new ClaudeAgentModel({ client }).step(request);
...
test("returns tool calls as blocks, and counts cached input in the prompt total", async () => {
  ...
  const step = await new ClaudeAgentModel({ client }).step(request);
...
test("prices Opus 5: cache reads at a tenth, cache writes at 1.25×", async () => {
  const model = new ClaudeAgentModel({ client: clientReturning(toolUseMessage).client });
  ...
  expect(new ClaudeAgentModel({ model: "claude-unknown", client: clientReturning(toolUseMessage).client }).cost({ promptTokens: 1, cacheHitTokens: 0, completionTokens: 1 })).toBeNull();
});
...
test("the abort signal reaches the SDK", async () => {
  ...
  await new ClaudeAgentModel({ client }).step({ ...request, signal });
