...
import { bubbleHint, userEnergy } from "../src/companion/energy.ts";
...
test("the bubble hint goes last, and never in a crisis", () => {
  ...
  expect(buildSystemPrompt(parts, { ...base, crisis: true, energy: "low" })).not.toContain("这一轮回几条");
  expect(buildSystemPrompt(parts, { ...base, crisis: false, energy: "high" })).not.toContain("这一轮回几条");
});
